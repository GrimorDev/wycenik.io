...
export function DashboardSidebar({ email, usage }: { email: string; usage: PlanUsage }) {
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col gap-6 bg-slate-900 px-4 py-5 md:w-60 md:min-h-screen">
      <Link href="/dashboard" className="flex items-center gap-2 px-1">
        <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-emerald-500 text-white">
          <BoltIcon className="h-4 w-4" />
        </span>
        <span className="text-base font-semibold text-white">
          Wycenik<span className="text-emerald-400">.io</span>
        </span>
      </Link>
      ...
      <div className="mt-auto flex flex-col gap-4">
        <div className="flex items-center justify-between gap-2 border-t border-slate-800 px-1 pt-4">
          <span className="truncate text-xs text-slate-500">{email}</span>
          <div className="flex shrink-0 items-center gap-2.5">
            <Link href="/dashboard/account" title="Ustawienia konta" className="text-slate-500 hover:text-slate-300">
              <GearIcon className="h-3.5 w-3.5" />
            </Link>
            <LogoutButton className="text-xs text-slate-500 hover:text-slate-300" />
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-800/40 p-4">
          ...
          <Link
            ...
            className="mt-4 flex w-full items-center justify-center rounded-lg bg-emerald-500 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-emerald-600"
          >
            Zwiększ limity
          </Link>
        </div>
